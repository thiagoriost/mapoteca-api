...
import {
  DirectusService
} from '../directus/directus.service';

import {
  MAPOTECA_CONSTANTS
} from '../../../../common/constants/mapoteca.constants';
 ...
 */
  ...
  async obtenerTematicas():
    ...
    const subcarpetas =
      await this.obtenerTematicasMapoteca();
 ...
 */
  async obtenerDocumentos(
    ...
    if (filters.sort) {
      ...
      const direction =
        filters.direction?.toLowerCase() === 'desc'
          ? -1
          : 1;

      documentosFiltrados.sort(
   ...
   */
  private async obtenerTematicasMapoteca():
    ...
    if (!carpetas.length) {
      ...
      throw new NotFoundException(
        'No existe la carpeta principal Mapoteca.'
      );
   ...
   * puede imprimirse.
   */
  private esImprimible(
    mimeType: string
  ): boolean {

    return (
      MAPOTECA_CONSTANTS
        .PRINTABLE_TYPES as readonly string[]
    ).includes(
      mimeType
    );
  ...
  async obtenerUrlDescarga(
     ...
     */
    await this.obtenerDocumentoPorId(
      documentId
    );

    return this.directusConfig
      .getDownloadUrl(
        documentId
      );
